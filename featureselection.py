...
def driver():
    ...
    # Getting the number of instances by doing a summation of all the lines in the file. 
    number_of_instances = sum(1 for line in file)

    # Preventing an out of bounds error when accessing a list out of range
    file.seek(0)

    # Quick traversing by separating each line of the file into the list itself. 
...
def nearest_neighboring(file, set_of_features, number_of_instances, label_object):
    ...
    for i in range(number_of_instances):
        ...
        # Comparison of neighboring distance and label object to classify.  
...
def leave_one_out_cross_validation(file, set_of_features, number_of_instances):
    ...
    for i in range (number_of_instances):
        ...
        # If the label object to classify is equal to the nearest neighbor "label", then add one to the correctly classified number 
